...
class EVAgent(PokerPlayer):
    ...
    def act(self,return_action=False): #Acting function
        active_players = 0
        Facing_bet = -1
        Facing_player = -1
        ...
        if(len(self.game.board) == 0):
            ...
            if(action == "check/call" and self.can_check_call() or self.check_call_amount == 0):
                ...
                if return_action:
                    return act_dis
            elif (action == "bet") and (not self.can_bet_raise()):
                f = self.check_call
                act_dis = encode_action(f,self)
                f()
                if return_action:
                ...
                if(f != self.check_call):
                    ...
                    for player in self.game.players:
                        if(player != self.game.actor and player.stack >= 4): 
                            Min_stacks = False
                    if(Min_stacks): #Cannot bet when opp. stack size gets too low
                        ...
                        # print('amount ', amount)
                        amount = max(amount, self.bet_raise_min_amount)
                        act_dis = encode_action(amount,self)
                        f(amount)
                        if return_action:
                            return act_dis
                else:
    ...
    def HandEliminator(self): #The idea behind this function would be to narrow down ranges for post-flop play. This Function is not used
        for i,player in enumerate(self.game.players):
            if(player.is_active() and player != self.game.actor):
                possible_hands = []
                for z in EVhands.possible_hands[i]:
                    cards = []
                    cards2 = []
                    cards.append(z[0])
                    cards.append(z[1])
